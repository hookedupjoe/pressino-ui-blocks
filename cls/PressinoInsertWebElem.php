<?php
/**
 * Class related to server side widget: PressinoInsertWebElem
 * 
...
if ( ! class_exists( 'PressinoInsertWebElem' ) ) {
	/**
	 * Class PressinoInsertWebElem
	 */
	class PressinoInsertWebElem {
		/**
		...
		public function class_init() {
			...
			add_shortcode( 'insert-web-elem', array( $this, 'actapp_handle_shortcode_insert_web_elem' ) );
			self::custom_post_website_element();
		}
	...
	private static function custom_post_website_element() {
		...
		public function block_render_callback( $attr ) {
			...
			$shortcode = sprintf(
				'[insert-web-elem page="%1$s" display="%2$s"%3$s%4$s%5$s%6$s%7$s%8$s]',
		 ...
		 */
	    public function actapp_handle_shortcode_insert_web_elem( $atts, $content = null ) {
...
$insert_web_elem_pages_plugin = null;

// Initialize InsertPagesPlugin object.
if ( class_exists( 'PressinoInsertWebElem' ) ) {
	$insert_web_elem_pages_plugin = PressinoInsertWebElem::get_instance();
}

add_action('init', array( $insert_web_elem_pages_plugin, 'class_init'));
add_filter(
	'template_include',
	array( 'PressinoInsertWebElem', 'override_tpl')
);